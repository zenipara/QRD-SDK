...
unsafe impl Send for QrdSchema {}
...
pub extern "C" fn qrd_schema_builder_add_field(
    ...
    // Take ownership, call add_field, and put back
    match builder_ref.0.take() {
        Some(inner) => {
            match inner.add_field(&name, ft, null) {
                Ok(new_inner) => {
                    builder_ref.0 = Some(new_inner);
                    0
                }
                Err(e) => { 
...
#[no_mangle]
pub extern "C" fn qrd_schema_field_count(schema: *const QrdSchema) -> c_uint {
    if schema.is_null() {
        return 0;
    }
    unsafe { (*schema).0.fields.len() as c_uint }
}
...
pub extern "C" fn qrd_reader_row_count(reader: *const QrdReader) -> c_uint {
    unsafe { (*reader).0.row_count() as c_uint }
}
