...
use qrd_core::encoding::{
    bit_packed::BitPackedEncoder, delta_binary::DeltaBinaryEncoder, plain::PlainEncoder,
    rle::RleEncoder, Encoder,
};
...
fn encoding_benchmarks(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        let random_bytes: Vec<u8> = random_data
            .iter()
        ...
        let repetitive_bytes: Vec<u8> = repetitive_data
            .iter()
            .flat_map(|value| value.to_le_bytes())
            .collect();
        ...
        let delta_bytes: Vec<u8> = delta_data
            .iter()
            .flat_map(|value| value.to_le_bytes())
            .collect();

        group.bench_with_input(
            BenchmarkId::new("plain/random", size),
            &random_data,
            |b, data| {
                b.iter(|| {
        ...
        group.bench_with_input(
            BenchmarkId::new("rle/repetitive", size),
            &repetitive_data,
            |b, data| {
                b.iter(|| {
        ...
        group.bench_with_input(
            BenchmarkId::new("delta_binary/sequential", size),
            &delta_data,
            |b, data| {
                b.iter(|| {
        ...
        group.bench_with_input(
            BenchmarkId::new("bit_packed/random", size),
            &random_data,
            |b, data| {
                b.iter(|| {
                    let encoder = BitPackedEncoder::new();
                    let mut output = Vec::new();
                    let boolean_bytes: Vec<u8> =
                        data.iter().map(|value| (value & 1) as u8).collect();
...
fn decoding_benchmarks(c: &mut Criterion) {
    ...
    let original_bytes: Vec<u8> = original_data
        .iter()
        .flat_map(|value| value.to_le_bytes())
        .collect();
    let boolean_bytes: Vec<u8> = original_data
        .iter()
    ...
    group.bench_function("plain/decode", |b| {
        b.iter(|| {
            ...
            let output = decoder
                .decode(&plain_encoded, original_bytes.len())
                .unwrap();
            black_box(output);
        });
    });
    ...
    group.bench_function("delta_binary/decode", |b| {
        b.iter(|| {
            ...
            let output = decoder
                .decode(&delta_encoded, original_bytes.len())
                .unwrap();
            black_box(output);
        });
    });
    ...
    group.bench_function("bit_packed/decode", |b| {
        b.iter(|| {
            ...
            let output = decoder
                .decode(&bitpacked_encoded, boolean_bytes.len())
                .unwrap();
            black_box(output);
        });
    });
...
fn compression_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("zstd/level1", |b| {
        b.iter(|| {
            let compressed =
                compress(&data, CompressionCodec::Zstd, CompressionLevel::new(1)).unwrap();
    ...
    group.bench_function("zstd/level6", |b| {
        b.iter(|| {
            let compressed =
                compress(&data, CompressionCodec::Zstd, CompressionLevel::new(6)).unwrap();
            black_box(compressed);
        });
    });
    ...
    group.bench_function("lz4/default", |b| {
        b.iter(|| {
            let compressed =
                compress(&data, CompressionCodec::Lz4, CompressionLevel::new(4)).unwrap();
            black_box(compressed);
        });
    });
