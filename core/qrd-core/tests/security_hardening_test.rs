...
use qrd_core::{
    ...
    error::Error,
    prelude::{FieldType, Nullability, SchemaBuilder},
    validation::{CorruptionDetector, CorruptionType, Validator},
};
...
fn s1_1_aes_256_gcm_nonce_uniqueness() {
    ...
    assert_ne!(
        ...
        "Nonces should be unique, ciphertexts must differ"
    );
...
fn s2_3_invalid_offset_detection() {
    ...
    assert!(
        result.is_none(),
        "Valid offset should not report corruption"
    );
...
fn s3_4_column_count_bounds() {
    ...
    let schema_builder = schema_builder
        .add_field("col1".to_string(), FieldType::Int64, Nullability::Required)
        .unwrap();
...
fn s4_1_encryption_error_messages() {
    ...
    if let Some(Error::ConfigError(msg)) = error {
        assert!(
            ...
            "Error message should mention byte requirement"
        );
    }
}
...
fn integration_schema_validation_with_encryption() {
    ...
    let schema_builder = SchemaBuilder::new();
    let schema_builder = schema_builder
        .add_field("id".to_string(), FieldType::Int64, Nullability::Required)
        .unwrap();
    let schema_builder = schema_builder
        .add_field("name".to_string(), FieldType::String, Nullability::Optional)
        .unwrap();
    let _schema = schema_builder.build().unwrap();
