...
fn test_writer_reader_consistency_small() {
    ...
    let schema = SchemaBuilder::new()
        ...
        .unwrap()
        .add_field("b", FieldType::Int32, Nullability::Required)
        .unwrap()
        .build()
        .unwrap();
...
fn test_writer_reader_consistency_medium() {
    let temp = NamedTempFile::new().unwrap();
    let schema = SchemaBuilder::new()
        .add_field("x", FieldType::Int64, Nullability::Required)
        .unwrap()
        .add_field("y", FieldType::Blob, Nullability::Optional)
        .unwrap()
        .build()
        .unwrap();
    ...
    for i in 0..5000 {
        ...
        writer
            .write_row(vec![(i as i64).to_le_bytes().to_vec(), blob])
            .unwrap();
    }
    writer.finish().unwrap();
...
fn test_writer_reader_many_small_files() {
    for _ in 0..20 {
        let temp = NamedTempFile::new().unwrap();
        let schema = SchemaBuilder::new()
            .add_field("v", FieldType::Int32, Nullability::Required)
            .unwrap()
            .build()
            .unwrap();
...
fn test_writer_reader_alternating_types() {
    ...
    let schema = SchemaBuilder::new()
        ...
        .unwrap()
        .add_field("f", FieldType::Float32, Nullability::Required)
        .unwrap()
    ...
    for i in 0..1000 {
        let mut row = vec![
            (i as i32).to_le_bytes().to_vec(),
            ((i as f32) * 1.1).to_le_bytes().to_vec(),
        ];
        if i % 3 == 0 {
            let mut s = (3u32).to_le_bytes().to_vec();
