...
fn test_schema_many_fields() {
    ...
    for i in 0..30 {
        b = b.add_field(&format!("col{}", i), FieldType::Int32, Nullability::Required).unwrap();
    }
...
fn test_varint_edge_large() {
    ...
    let enc = varint::encode(v);
    // decode similarly
    let mut val = 0u64; let mut shift = 0u32;
    for b in enc.iter() {
        val |= ((b & 0x7F) as u64) << shift;
        if b & 0x80 == 0 { break; }
        shift += 7;
...
fn test_schema_optional_fields() {
    let s = SchemaBuilder::new()
        .add_field("o", FieldType::String, Nullability::Optional).unwrap()
        .add_field("r", FieldType::Int32, Nullability::Required).unwrap()
        .build().unwrap();
...
fn test_footer_roundtrip_with_metadata_index_none() {
    ...
    let schema = SchemaBuilder::new().add_field("x", FieldType::Blob, Nullability::Required).unwrap().build().unwrap();
