...
check_docs() {
  ...
  if cargo doc --workspace --all-features --no-deps --document-private-items 2>&1; then
    local duration=$(calculate_duration "$start_time")
    log_pass "cargo doc completed in ${duration}s"
    return 0
  else
    local duration=$(calculate_duration "$start_time")
    log_fail "cargo doc failed after ${duration}s"
    return 1
  fi
}
