...
check_integration() {
  ...
  if cargo test --workspace --test "*" --all-features 2>&1; then
    local duration=$(calculate_duration "$start_time")
    log_pass "integration tests completed in ${duration}s"
    return 0
  else
    local duration=$(calculate_duration "$start_time")
    log_fail "integration tests failed after ${duration}s"
    return 1
  fi
}
