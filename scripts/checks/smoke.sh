...
check_smoke() {
  ...
  # Run a quick smoke test on core functionality
  cd "$REPO_ROOT/core/qrd-core"
  if cargo test --lib --quiet smoke 2>&1; then
    local duration=$(calculate_duration "$start_time")
    log_pass "smoke tests completed in ${duration}s"
    return 0
  else
    local duration=$(calculate_duration "$start_time")
    log_fail "smoke tests failed after ${duration}s"
    return 1
  fi
}
